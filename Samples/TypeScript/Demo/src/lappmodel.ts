...
/**
 * ユーザーが実際に使用するモデルの実装クラス<br> 用户实际使用的模型的实现类
 * モデル生成、機能コンポーネント生成、更新処理とレンダリングの呼び出しを行う。模型生成，功能组件生成，更新处理和渲染调用。
 */
export class LAppModel extends CubismUserModel {
  ...
  /**
   * 引数で指定したモーションの再生を開始する 开始播放由参数指定的运动
   * @param group モーショングループ名 名称
   * @param no グループ内の番号 组内号码
   * @param priority 優先度 优先级
   * @param onFinishedMotionHandler モーション再生終了時に呼び出されるコールバック関数 运动再现结束时调用的回调函数
  ...
  /**
   * ランダムに選ばれたモーションの再生を開始する。 随机播放运动
   * @param group モーショングループ 名称
   * @param priority 優先度
 ...
 /**
  ...
  /**
   * ランダムに選ばれた表情モーションをセットする 设置随机选择的表情动作
   */
  ...
  /**
   * モーションデータをグループ名から一括でロードする。从组名统一加载运动数据。
   * モーションデータの名前は内部でModelSettingから取得する。 运动数据的名称在内部从ModelSetting获取。
   *
